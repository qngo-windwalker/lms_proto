...
class OpignoGroupManagerController extends ControllerBase {
  ...
  public static function ajaxFormEntityCallback(&$form, FormStateInterface $form_state) {
    ...
    $file = $entity->hasField('field_course_image')
      ? File::load($entity->get('field_course_image')->getValue()[0]['target_id'])
      : NULL;
    ...
    $item['title'] = $entity->hasField('label')
      ? $entity->get('label')->getString() : NULL;

    if (!$item['title'] && $entity->hasField('name')) {
      $item['title'] = $entity->get('name')->getString();
    }
   ...
   */
  public function getItemTypes(Group $group, $mainItem = NULL, $json_output = TRUE) {
    // Init the return array and get all the content types available.
    $available_types = [];
    $content_types_definitions = $this->content_types_manager->getDefinitions();
