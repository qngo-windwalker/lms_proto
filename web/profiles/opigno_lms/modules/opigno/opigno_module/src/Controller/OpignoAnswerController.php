...
class OpignoAnswerController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionPageTitle($opigno_answer_revision) {
    $opigno_answer = $this->entityManager()->getStorage('opigno_answer')->loadRevision($opigno_answer_revision);
    return $this->t('Revision of %title from %date', array('%title' => $opigno_answer->label(), '%date' => format_date($opigno_answer->getRevisionCreationTime())));
  }
  ...
  public function revisionOverview(OpignoAnswerInterface $opigno_answer) {
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              ...
              Url::fromRoute('entity.opigno_answer.translation_revert', ['opigno_answer' => $opigno_answer->id(), 'opigno_answer_revision' => $vid, 'langcode' => $langcode]) :
              Url::fromRoute('entity.opigno_answer.revision_revert', ['opigno_answer' => $opigno_answer->id(), 'opigno_answer_revision' => $vid]),
            ];
          }
    ...
    $build['opigno_answer_revisions_table'] = array(
      '#theme' => 'table',
      '#rows' => $rows,
      '#header' => $header,
    );
