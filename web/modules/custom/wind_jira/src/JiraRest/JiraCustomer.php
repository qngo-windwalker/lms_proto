...
use Drupal\wind_jira\JiraRest\JiraCustomerService;

/**
 * Class JiraCustomer
 * @package Drupal\wind_jira\JiraRest
 */
class JiraCustomer extends GenericJiraObject implements IGenericJiraObjectRoot {

  /**
   * Reference to the OrganizationService that generated this organization.
   * @var \Drupal\wind_jira\JiraRest\JiraCustomerService
   */
   ...
   */
  public function __construct(JiraCustomerService $customerService, GenericJiraObject $initObject = null, $isLoaded = false, $parent = null) {
    parent::__construct();
  ...
  public function save() {
    ...
    if ($this->persistent) {
      ...
      if (!empty((array) $this->getDiffObject())) {
        ...
        $response = $this->customerService->getCommunicationService()->put($path, $this->getDiffObject());
        if ($response !== false) {
    ...
    // create if this issue was not yet persistent in jira
      ...
      if ($this->hasRequiredCreateProperties()) {
        $response = $this->customerService->getCommunicationService()->post('customer', $this->getDiffObject());
    ...
    if (empty($this->parent)) {  // only issues without parents can have sub tasks
      ...
      if (!empty($sub_issue_array) && empty($this->subIssues)) {
        ...
        foreach ($sub_issue_array as $sub_issue) {
          $this->subIssues[$sub_issue->key] = new Issue($this->customerService, $sub_issue, false, $this);
        }
  ...
  public function createSubIssue() {
    ...
    if (empty($this->parent)) { // only create sub task if this issue has no parent
      $sub_issue = new Issue($this->customerService, null, false, $this);
...
//      if (!$key_exists || !$id_exists) {
      ...
      if (!empty($this->fields->parent)) {
        $this->parent = new Issue($this->customerService, $this->fields->parent);
      }
    }
  }
...
//    $this->addDiffableObject('name', 'tseting');
  ...
  private function hasRequiredCreateProperties() {
    ...
    $DisplayName_exists = !empty($diffObject->displayName);
    $email_exists = !empty($diffObject->email);
  ...
  public function loadData() {
    if ($this->persistent && !$this->loaded) {
      if (!empty($this->key) || !empty($this->id)) {
        ...
        $response = $this->customerService->getCommunicationService()->get('customer/' . $key);
  ...
  public function getCommunicationService() {
    return $this->customerService->getCommunicationService();
  }
