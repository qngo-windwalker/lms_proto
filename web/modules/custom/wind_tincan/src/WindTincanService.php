...
namespace Drupal\wind_tincan;

use Drupal\Core\Database\Connection;
use Drupal\file\Entity\File;

/**
 * @see Drupal\opigno_tincan_activity\TincanService
 *
 * Class WindTincanService.
 */
class WindTincanService {

  const PATH_PUBLIC_PACKAGE_FOLDER = 'public://tincan/';

  const PATH_PUBLIC_EXTRACTED_PACKAGE_FOLDER = 'public://tincan_extracted/';
  ...
  public function saveTincanPackageInfo(File $file) {
    ...
    if ($package_info === FALSE) {
      \Drupal::logger('wind_tincan_activity')->error(
    ...
    try {
      return $connection->insert('wind_tincan_type_properties')
        ->fields([
          'fid' => $fid,
  ...
  public function tincanLoadByFileEntity(File $file) {
    $connection = $this->connection;
    return $connection->select('wind_tincan_type_properties', 'otn')
  ...
  public static function unzipPackage($file) {
    ...
    if ($result === TRUE) {
      ...
      \Drupal::logger('wind_tincan_activity')
