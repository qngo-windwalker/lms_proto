...
class WindTincanCertificateIdController extends ControllerBase{

  public function getContent($statement_id) {
    ...
    if (!$result) {
      return [
        'content' => [
          '#markup' => "<h3>There was an issue locating your certificate.</h3>"
        ],
      ];
    }

    $statement = TincanStatement::load($this->array_first_child_value($result));
    $jsonValue = $statement->get('json')->value;
    $json_array = Json::decode($jsonValue);
    $fullName = $this->getUserFullName($json_array['actor']['account']);
...
//    $completionData = '<span style="font-size:12pt;">'. date('m/d/Y', $json_array['timestamp'] . '</span>';
    ...
    $statementIdMarkup = '<span style="font-size:12pt;">' . $statement_id . '</span>';
    $pdf->writeHTMLCell(0, 0, '98', '185', $statementIdMarkup, 0, 1, 0, true, '', true);
//    $pdf->writeHTML($html, true, false, true, false, '');
    // Close and output PDF document
    // This method has several options, check the source code documentation for more information.
    $pdf->Output('clearinghousenavigation_certificate_001.pdf', 'I');
  }
