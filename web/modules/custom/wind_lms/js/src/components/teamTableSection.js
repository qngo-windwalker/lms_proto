...
export default class TeamTableSection extends Component{
  constructor(props) {
    super(props);
    this.state = {
      items: [],
  ...
  render(){
    return (
      <div className="section">
        ...
        {this.state.isLoaded && <TeamDatatable columns={this.columns} data={this.state.items} deleteRow={this.deleteRow}  />}
  ...
  getOperationsContent(row, type, val, meta){
    let markup = ReactDOMServer.renderToString(
      <div className={`btn-group`}>
        ...
        <ExportCSV data={Utility.getAllActiveUsersInTeam(row.tid, this.props.data.userData)} />
      </div>
    );
